...
class ParallelTests(unittest.TestCase):
 ...
 def test_groups_chrome_profile_serially_and_launches_generic_apps(self):
  ...
  ws=[dict(id=1,app='Google Chrome',reopen=src('b')),dict(id=2,app='Google Chrome',reopen=src('c')),dict(id=3,app='Docker Desktop')]
  ...
  with patch.object(m,'run_open_jobs',side_effect=jobs),patch.object(m,'progress'),patch.object(m,'open_missing_apps',return_value={'opened':['Docker Desktop']}) as generic,patch.object(m,'reopen_windows',return_value={'reopened_windows':['Chrome']}) as reopen,patch.object(m.e,'windows',return_value={}),patch.object(m,'with_bundles',return_value=[]),patch.object(m.adapters,'annotate',return_value=([],[])),patch.object(m,'match',return_value=({1:1,2:2,3:3},{})),patch.object(m.time,'sleep'):
   result=m.open_parallel(document,[],{},1)
   self.assertFalse(generic.call_args.kwargs['wait_ready'])
